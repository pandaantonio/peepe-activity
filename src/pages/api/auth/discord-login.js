...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Pega o id interno gerado na listagem
    const dbKey = Object.keys(usersData)[0];
    const user = usersData[dbKey];

    return res.status(200).json({ 
      message: 'Logado automaticamente via Discord!', 
      username: user.username, // Retorna o username guardado originalmente (com o ponto se houver)
