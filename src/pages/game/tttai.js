...
export default function TicTacToe() {
  ...
  const boardCells = useMemo(() => board.map((row, rIdx) =>
    row.map((cell, cIdx) => (
      <GridCell
        key={`${rIdx}-${cIdx}`}
        cell={cell} rowIndex={rIdx} colIndex={cIdx}
  ...
  return (
    <div className="min-h-[100dvh] bg-[#0a0a0c] text-[#ededed] select-none font-sans antialiased flex flex-col relative overflow-hidden">
      ...
      <header className="bg-white/[0.02] backdrop-blur-xl border-b border-white/5 shrink-0 z-20 relative">
        <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 py-2 sm:py-3 flex items-center justify-between gap-2">
          ...
          <div className="flex-1 flex justify-center min-w-0 px-1">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/[0.02] border border-white/10 text-[10px] sm:text-xs font-bold uppercase tracking-wider max-w-full">
      ...
      {/* ── MAIN: layout adaptativo ── */}
      <main className="flex-1 w-full max-w-7xl mx-auto px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-center z-10 relative min-h-0">

        {/*
          Portrait  → coluna: tabuleiro em cima, placar embaixo
          Landscape → linha:  tabuleiro à esquerda, placar à direita
        */}
        <div className="ttt-layout w-full h-full">
          ...
          >
            ...
            {(winner || isDraw) && (
              <div
                role="dialog" aria-modal="true" aria-label="Fim de jogo"
              ...
              >
                <div className={`w-14 h-14 sm:w-18 sm:h-18 rounded-2xl flex items-center justify-center shadow-2xl ${
                  winner === 'X' ? 'bg-emerald-500/10 border border-emerald-500/30' :
                  winner === 'O' ? 'bg-rose-500/10 border border-rose-500/30' :
                  'bg-amber-500/10 border border-amber-500/30'
                }`}>
                  <span className={`text-3xl sm:text-5xl font-black drop-shadow-md ${
                ...
                <div className="text-center max-w-[220px] sm:max-w-sm">
                  <h2 className={`text-lg sm:text-2xl font-black mb-1 tracking-wide ${
                    winner === 'X' ? 'text-emerald-400' :
                  ...
                  <p className="text-gray-400 text-[11px] sm:text-sm leading-relaxed">
                    ...
                    {isDraw  && 'Nenhum sistema obteve vantagem espacial.'}
                  </p>
                </div>
          ...
          {/* ── PLACAR / PAINEL LATERAL ── */}
          <aside className="ttt-panel glass-card rounded-2xl p-3 sm:p-5 shadow-2xl flex flex-col justify-between gap-3">

            {/* Título */}
            <div className="flex flex-col gap-0.5 flex-shrink-0">
              <span className="text-[9px] sm:text-[10px] font-bold text-emerald-400 uppercase tracking-widest">Módulo do Placar</span>
              <h2 className="text-sm sm:text-base font-black text-white leading-tight">Consola de Desempenho</h2>
            </div>

            {/* Placar */}
            <div className="bg-[#050507]/60 rounded-xl p-2.5 sm:p-4 border border-white/5 shadow-inner backdrop-blur-md flex-shrink-0">
              <div className="grid grid-cols-3 gap-1 items-center text-center">
                ...
                <div className="flex flex-col items-center gap-1">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center">
                    <FaUser className="text-emerald-400" size={12} />
                  </div>
                  <span className="text-[8px] sm:text-[9px] text-gray-500 font-bold uppercase tracking-wider leading-tight">Você (X)</span>
                  <span className="text-lg sm:text-2xl font-black text-white">{scores.player}</span>
                </div>

                {/* Empates */}
                <div className="flex flex-col items-center gap-0.5 border-x border-white/5 px-1">
                  <span className="text-[8px] sm:text-[9px] text-gray-600 font-bold uppercase tracking-wider">Empates</span>
                  <span className="text-base sm:text-xl font-black text-amber-400/90">{scores.draws}</span>
                  <span className="text-[8px] sm:text-[9px] text-gray-600">{totalGames} partidas</span>
                </div>

                {/* IA */}
                <div className="flex flex-col items-center gap-1">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center">
                    <FaRobot className="text-rose-400" size={12} />
                  </div>
                  <span className="text-[8px] sm:text-[9px] text-gray-500 font-bold uppercase tracking-wider leading-tight">IA (O)</span>
                  <span className="text-lg sm:text-2xl font-black text-white">{scores.ai}</span>
                </div>
              </div>

              {/* Barra distribuição */}
              <div className="mt-3 flex h-1.5 rounded-full overflow-hidden bg-white/5 shadow-inner">
                <div className="bg-emerald-400/80 transition-all duration-500 shadow-[0_0_8px_rgba(52,211,153,0.5)]"
                  style={{ width: `${totalGames > 0 ? (scores.player / totalGames) * 100 : 33.3}%` }} />
                <div className="bg-amber-400/60 transition-all duration-500"
                  style={{ width: `${totalGames > 0 ? (scores.draws / totalGames) * 100 : 33.4}%` }} />
                <div className="bg-rose-500/80 transition-all duration-500 shadow-[0_0_8px_rgba(244,63,94,0.5)]"
                  style={{ width: `${totalGames > 0 ? (scores.ai / totalGames) * 100 : 33.3}%` }} />
              </div>
            </div>

            {/* Dica algoritmo */}
            <div className="p-2.5 sm:p-3 bg-purple-500/[0.02] border border-white/5 border-l-4 border-l-purple-500/40 rounded-r-xl flex items-start gap-2 flex-shrink-0">
              <FaTrophy size={12} className="text-purple-400 mt-0.5 flex-shrink-0" />
              <div className="text-left min-w-0">
                <span className="font-bold text-purple-400/80 uppercase tracking-wider text-[9px] sm:text-[10px]">Algoritmo</span>
                <p className="text-gray-400 mt-0.5 leading-relaxed text-[10px] sm:text-xs">
            ...
            <div className="flex gap-2 flex-shrink-0">
              ...
              >
                <FaTrashAlt size={10} /> Zerar Placar
              </button>
            </div>
